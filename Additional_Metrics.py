...
class CryptoAnalyzer:
    ...
    def get_derivatives_data(self):
        try:
            ...
            # Get Open Interest
            oi_response = requests.get(f"{binance_futures_url}/openInterest", params={'symbol': 'BTCUSDT'})
    ...
    def get_all_metrics(self):
        ...
        print(f"correlation_data: {self.get_correlation_data()}")
        print(f"technical_indicators: {technical.tail(1).to_dict()}")
        print(f"onchain_metrics: {self.get_onchain_metrics()}")
        print(f"market_sentiment: {self.get_market_sentiment()}")
        print(f"whale_activity: {self.get_whale_activity()}")
        print(f"derivatives_data: {self.get_derivatives_data()}")
        print(f"additional_metrics: {self.get_additional_metrics()}")

        tracker = CryptoLiquidationTracker()
        data = tracker.get_aggregated_liquidation_data()
        print(f"liquidation_data: {data}")

        print(f"market_depth: {self.get_market_depth()}")
